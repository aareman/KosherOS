...
# Install (or repair) this repository's shared git hooks.
#
...
# overwritten a hook. Git keeps ONE hooks directory for a repository and
# all of its worktrees, so a hook must never name a checkout — see
# scripts/git-hook-dispatch.sh. It is installed under every stage devenv
# uses (pre-commit, pre-push), and any other prek-generated shim found
# there is replaced, since every one of them carries a baked-in path.
hooks:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    for name in pre-commit pre-push; do
        install -m 755 scripts/git-hook-dispatch.sh "$hooks/$name"
    done
    for hook in "$hooks"/*; do
        case "$hook" in *.sample|*.legacy) continue ;; esac
        if [ -f "$hook" ] && grep -q "File generated by prek" "$hook"; then
            install -m 755 scripts/git-hook-dispatch.sh "$hook"
        fi
    done
    for legacy in "$hooks"/*.legacy; do
        [ -f "$legacy" ] && cmp -s scripts/git-hook-dispatch.sh "$legacy" && rm -f "$legacy" || true
    done
    echo "git hooks installed into $hooks"
    if [ ! -f .pre-commit-config.yaml ]; then
        echo "note: this checkout has no .pre-commit-config.yaml yet, so the"
        echo "      hooks will do nothing. Enter the dev shell to generate it."
    fi
...
build:
    podman build --build-arg "KOSHER_VERSION=$(python3 scripts/version.py show)" -t {{image}} -f os-image/Containerfile .
