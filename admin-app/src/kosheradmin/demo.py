...
family, and every control works on the sample data, so a change made on
one screen shows on the others. Nothing touches the machine. The family
is the one from the design canvas (docs/design/), which is what the real
screens were drawn against.
"""
...
class DemoClient:
    ...
    def __init__(self):
        ...
        self.installed = [
            {"ref": "org.gimp.GIMP", "name": "GIMP", "installed_by": "avi", "approved": True},
            {"ref": "org.libreoffice.LibreOffice", "name": "LibreOffice", "installed_by": "",
             "approved": True},
        ]
        self.catalog = [{"ref": f"org.example.App{i}", "name": f"Example app {i}",
                         "summary": "An approved app"} for i in range(23)]
    ...
    def install_app(self, ref): return None
    ...
    def connect_app_signals(self, on_progress, on_finished): return 0
