...
in
{
  ...
  # The install itself is a task ordered after devenv's own hook installer
  # (devenv:git-hooks:install) and before enterShell, so the dispatcher is
  # what ends up in the hooks directory however the shell is entered —
  # direnv reload, `devenv shell`, a tasks-only run, a new devenv version.
  # The day the earlier enterShell-only version was merged, prek's shims
  # were found installed over it and every commit failed on the .legacy
  # name; the dispatcher now also tolerates that name, but it should not
  # have to.
  tasks."kosher:git-hooks:dispatch" = {
    description = "install the worktree-safe git hook dispatcher over prek's shims";
