...
check "root is unrestricted"                    0 $CURL https://fedoraproject.org
check "no-internet user is blocked"             1 as nokid $CURL https://example.com
check "dns-filtered user browses"               0 as dnskid $CURL https://example.com
check "whitelisted site loads"                  0 as wlkid $CURL https://example.com
check "non-whitelisted site is blocked"         1 as wlkid $CURL https://www.wikipedia.org
check "direct-IP browsing is blocked"           1 as wlkid $CURL https://93.184.216.34
...
check "an unmanaged account has no internet" 1 as unmanaged $CURL https://example.com
...
check "the window lets that user out"    0 as nokid $CURL https://example.com
